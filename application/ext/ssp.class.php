...
class SSP {
	...
	static function select($db , $table , $kolom , $vkey){
			$query = " select ".$kolom." from ".$table." WHERE id = ".$vkey;
			$stmt = $db->query($query);
			if($stmt){
			$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
			}
			return $results[0][trim($kolom)];
	}
	/**
	 * Paging
	 *
	 ...
	 */
	static function simple ( $request,  $table, $primaryKey = 'id', $columns,$db ,$uhome)
	{
		$bindings = array();
		...
		return array(
			...
			"data"            => self::data_output( $columns, $data ,$db,$uhome)
		);
	}
